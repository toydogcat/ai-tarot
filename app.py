    ...
    if not dates:
        ...
        if not records:
            ...
            for record in reversed(records):
                ...
                with st.expander(
                    f"{status_icon} [{record['time_display']}] {record['question'][:40]}..."
                    if len(record['question']) > 40
                    else f"{status_icon} [{record['time_display']}] {record['question']}"
                ):
