...
修復 `history/` 資料夾中 `ai_status.interpretation` 為 `"error"` 的占卜紀錄。使用 `--fix-audio` 也可以為 `ai_status.audio` 為 `"error"` 或缺失語音的紀錄補件。根據該次紀錄中的使用者提問、占卜類型 (塔羅/易經) 和結果，自動呼叫 Gemini 與 TTS API 重新生成。
